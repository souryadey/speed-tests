...
def matvecmult_random(a=1000,b=1000, density=1):
    ...
    setup = (
            ...
            'mat = torch.randn({0},{1})*torch.as_tensor(adjmat_random(density={2},p={1},n={0}), dtype=torch.float32, device=device);'
...
def matmult_clashfree(a=1000,b=1000,c=1000, density1=1, density2=1):
    ...
    setup = (
            ...
            'device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu");'
            'mat1 = torch.randn({0},{1}) * torch.as_tensor(adjmat_clash_free(p={1}, fo=int(np.ceil({3}*{0})), n={0}, z={1}//10), dtype=torch.float32, device=device);'
            'mat2 = torch.randn({1},{2}) * torch.as_tensor(adjmat_clash_free(p={2}, fo=int(np.ceil({4}*{1})), n={1}, z={2}//10), dtype=torch.float32, device=device)'
            ).format(a,b,c,density1,density2)
